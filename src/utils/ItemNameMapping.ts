...
export class ItemNameNormalizer {
  ...
  hasMapping(name: string): boolean {
    const normalizedName = name.toLowerCase().trim();
    return this.chineseMapping.has(normalizedName) ||
           this.englishAliasMapping.has(normalizedName) ||
           this.isStandardEnglishItem(name); // 标准英文名称也认为是有效映射
  }
  ...
  private isStandardEnglishItem(name: string): boolean {
    ...
    const commonItemPatterns = [
      /^wooden_/, /^stone_/, /^iron_/, /^gold/, /^diamond_/, /^netherite_/,
      /_pickaxe$/, /_axe$/, /_sword$/, /_shovel$/, /_hoe$/,
      /^planks$/, /^stick$/, /^crafting_table$/, /^furnace$/,
      /^chest$/, /^coal$/, /^ingot$/, /^ore$/, /^log$/
    ];
