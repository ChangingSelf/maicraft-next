...
export class TemplateEngine {
   ...
   * 渲染模板
   */
  render(template: string, variables: Record<string, any> = {}, options?: Partial<RenderOptions>): RenderResult {
    try {
      const processResult = this.process(template);
      const renderer = options ? new Renderer({ ...this.config.renderer, ...options }) : this.renderer;
