...
export class Compiler {
  ...
  private compileNode(node: TemplateAST, context: CompileContext): void {
    switch (node.type) {
      ...
      case 'include':
        ...
        break;
      default:
        throw PromptErrorFactory.renderError(`Unknown node type: ${(node as any).type}`, undefined, undefined, undefined, {
    ...
    if (parsed.filters.length > 0) {
      for (const filter of parsed.filters) {
        if (this.options.filters[filter.name]) {
          ...
          throw PromptErrorFactory.renderError(`Unknown filter: ${filter.name}`, undefined, undefined, undefined, {
            filter: filter.name,
            position: node.position,
          });
        }
  ...
  private compileLoop(node: TemplateAST, context: CompileContext): void {
    ...
    try {
      ...
      throw PromptErrorFactory.renderError(`Invalid loop configuration: ${node.value}`, undefined, undefined, undefined, { position: node.position });
    }
