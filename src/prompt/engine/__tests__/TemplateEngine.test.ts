...
describe('TemplateEngine', () => {
  ...
  describe('循环渲染', () => {
    it('应该处理简单数组循环', () => {
      const result = engine.render('${#each items}${this}${/each}', { items: ['apple', 'banana', 'cherry'] });
      expect(result.result).toBe('applebananacherry');
    });

    it('应该处理带变量的循环', () => {
      const result = engine.render('${#each items in list}${item}: ${item}${/each}', { list: ['apple', 'banana'] });
      expect(result.result).toBe('apple: applebanana: banana');
    });

    it('应该处理循环索引变量', () => {
      const result = engine.render('${#each items}${item_index}: ${this}${/each}', { items: ['a', 'b', 'c'] });
      expect(result.result).toBe('0: a1: b2: c');
    });

    it('应该处理空数组', () => {
      const result = engine.render('${#each items}Nothing${/each}', { items: [] });
