...
export class Parser {
  ...
  parse(tokens: Token[]): TemplateAST {
    ...
    if (!this.isAtEnd()) {
      ...
      throw PromptErrorFactory.invalidTemplate(
  ...
  private parseNode(): TemplateAST | null {
    if (this.depth >= this.config.maxDepth) {
      throw PromptErrorFactory.invalidTemplate(
        `Maximum nesting depth (${this.config.maxDepth}) exceeded`
      )
    }
  ...
  private createVariableNode(token: Token): TemplateAST {
    if (!this.config.allowEmptyVariables && token.value.trim() === '') {
      throw PromptErrorFactory.invalidTemplate(
        `Empty variable at line ${token.line}, column ${token.column}`,
        { line: token.line, column: token.column }
      )
    }
  ...
  private parseConditional(startToken: Token): TemplateAST {
    ...
    while (!this.isAtEnd() && !this.check(TokenType.CONDITIONAL_END)) {
      if (this.check(TokenType.CONDITIONAL_ELSE)) {
        if (inElse) {
          throw PromptErrorFactory.invalidTemplate(
            `Multiple else blocks in conditional at line ${startToken.line}, column ${startToken.column}`,
    ...
    if (this.isAtEnd()) {
      throw PromptErrorFactory.invalidTemplate(
        `Unclosed conditional block starting at line ${startToken.line}, column ${startToken.column}`,
        { line: startToken.line, column: startToken.column }
      )
    }
  ...
  private parseLoop(startToken: Token): TemplateAST {
    ...
    if (loopExpr.includes(' in ')) {
      ...
      if (parts.length !== 2) {
        throw PromptErrorFactory.invalidTemplate(
          `Invalid loop expression: '${loopExpr}' at line ${startToken.line}, column ${startToken.column}`,
          { line: startToken.line, column: startToken.column }
        )
      }
    ...
    if (this.isAtEnd()) {
      throw PromptErrorFactory.invalidTemplate(
        `Unclosed loop block starting at line ${startToken.line}, column ${startToken.column}`,
        { line: startToken.line, column: startToken.column }
      )
    }
  ...
  private createIncludeNode(token: Token): TemplateAST {
    ...
    if (!templateName) {
      throw PromptErrorFactory.invalidTemplate(
        `Empty include directive at line ${token.line}, column ${token.column}`,
        { line: token.line, column: token.column }
      )
    }
