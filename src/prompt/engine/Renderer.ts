...
export class Renderer {
   ...
   * 渲染模板
   */
  render(compiledTemplate: CompiledTemplate, variables: Record<string, any> = {}): RenderResult {
    ...
    try {
      // 设置渲染超时
      const timeoutId =
        this.options.maxRenderTime > 0
          ? setTimeout(() => {
              throw PromptErrorFactory.renderError(`Template rendering timeout (${this.options.maxRenderTime}ms)`);
            }, this.options.maxRenderTime)
          : null;

      // 准备变量
      const processedVariables = this.processVariables(variables, compiledTemplate.dependencies, usedVariables, missingVariables);
    ...
    for (const dependency of dependencies) {
      ...
      if (value === undefined) {
        ...
        if (this.options.strict) {
          throw PromptErrorFactory.renderError(`Undefined variable: ${dependency}`, undefined, { variable: dependency });
        }
  ...
  validateDependencies(
    ...
    const missing = compiledTemplate.dependencies.filter(dep => !availableVariables.includes(dep));
    const extra = availableVariables.filter(variable => !compiledTemplate.dependencies.includes(variable));
  ...
  getRenderStatistics(results: RenderResult[]): {
    ...
    return {
      ...
      averageRenderTime: successful.length > 0 ? successful.reduce((sum, r) => sum + r.renderTime, 0) / successful.length : 0,
