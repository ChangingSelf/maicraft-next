...
describe('PromptManager', () => {
  ...
  describe('模板渲染测试', () => {
    ...
    test('应该能够渲染复杂模板', async () => {
      ...
      expect(result).toBeDefined();
      expect(result.rendered_content).toContain('Header: Shopping List');
      expect(result.rendered_content).toContain('Apples: 5');
      expect(result.rendered_content).toContain('Bananas: 3');
      expect(result.rendered_content).toContain('Footer');
    });
