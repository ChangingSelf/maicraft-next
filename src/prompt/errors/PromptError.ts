...
export class EnhancedPromptError extends Error {
  ...
  constructor(type: PromptErrorType, message: string, code?: string, details?: any, context?: Record<string, any>, cause?: Error) {
    super(message);
...
export class PromptErrorFactory {
   ...
   * 创建变量验证失败错误
   */
  static variableValidationFailed(errors: ValidationError[], context?: Record<string, any>): EnhancedPromptError {
    return new EnhancedPromptError(
   ...
   * 创建版本未找到错误
   */
  static versionNotFound(templateId: string, version: string, context?: Record<string, any>): EnhancedPromptError {
    return new EnhancedPromptError(
   ...
   * 创建存储错误
   */
  static storageError(message: string, operation?: string, path?: string, cause?: Error, context?: Record<string, any>): EnhancedPromptError {
    return new EnhancedPromptError(PromptErrorType.STORAGE_ERROR, `Storage error: ${message}`, 'STORAGE_ERROR', { operation, path }, context, cause);
  }

  /**
   * 创建缓存错误
   */
  static cacheError(message: string, operation?: string, key?: string, cause?: Error, context?: Record<string, any>): EnhancedPromptError {
    return new EnhancedPromptError(
   ...
   * 创建分类未找到错误
   */
  static categoryNotFound(id: string, context?: Record<string, any>): EnhancedPromptError {
    return new EnhancedPromptError(
   ...
   * 创建权限拒绝错误
   */
  static permissionDenied(action: string, userId?: string, resourceId?: string, context?: Record<string, any>): EnhancedPromptError {
    return new EnhancedPromptError(
...
export class ConsoleLogErrorHandler implements IErrorHandler {
  ...
  async handle(error: Error, context?: Record<string, any>): Promise<void> {
    const enhancedError =
      error instanceof EnhancedPromptError
        ? error
        : new EnhancedPromptError(PromptErrorType.RENDER_ERROR, error.message, 'UNKNOWN_ERROR', { originalError: error }, context);
...
export class ErrorManager {
   ...
   * 尝试恢复
   */
  async attemptRecovery(error: EnhancedPromptError, context?: Record<string, any>): Promise<boolean> {
   ...
   * 带重试的操作执行
   */
  async executeWithRetry<T>(operation: () => Promise<T>, context?: Record<string, any>): Promise<T> {
    let attempt = 1;
